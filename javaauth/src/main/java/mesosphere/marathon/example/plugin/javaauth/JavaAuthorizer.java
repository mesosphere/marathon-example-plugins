...
public class JavaAuthorizer implements Authorizer {
	...
	private boolean isAuthorized(JavaIdentity principal, Action action, PathId path) {
		switch (action) {
			case CreateAppOrGroup: return true;
			case UpdateAppOrGroup: return principal.getName().contains("ernie");
			case DeleteAppOrGroup: return path.toString().startsWith("/test");
			case ViewAppOrGroup: return true;
			case KillTask: return false;
